  ...
  switch (action.type) {
    ...
    case INGREDIENTS_REQUEST_SUCCESS: {
      return {
        ...
        requestFailed: false,
        dataApi: action.data,
        bun: action.data.filter((item: TIngredients) => item.type === "bun"),
        main: action.data.filter((item: TIngredients) => item.type === "main"),
        sauce: action.data.filter((item: TIngredients) => item.type === "sauce"),
        request: false,
      };
    }
